...
include("connect.php");
 ...
 $sql = "SELECT * FROM contact";  // Adjust the columns based on your needs
 ...
 if (mysqli_num_rows($result) > 0) {
    ...
    // Loop through each row and display the data
     while ($row = mysqli_fetch_assoc($result)) {
           ...
           echo "<td>$message</td>";
           echo "<td>$mobile</td>";
          //  echo '<td><a href="contact_delete.php?email=' . $email . '" class="btn btn-sm btn-danger">Delete</a></td>';
...
function deleteRecord() {
    if (modelIdToDelete) {
        window.location.href = `contact_delete.php?contact_id=${modelIdToDelete}`;
    }
}
</script>