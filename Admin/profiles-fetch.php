...
include("connect.php");
 ...
 $sql = "SELECT * FROM admins";  // Adjust the columns based on your needs
 ...
 if (mysqli_num_rows($result) > 0) {
    ...
    // Loop through each row and display the data
     while ($row = mysqli_fetch_assoc($result)) {
           ...
           echo "<td>$mobile</td>";
           echo "<td>$email</td>";
           echo '<td><a href="profile_update.php?profile_id=' . $id . '" class="btn btn-sm btn-success">Update</a></td>';
...
function deleteRecord() {
    if (modelIdToDelete) {
        window.location.href = `profile_delete.php?profile_id=${modelIdToDelete}`;
    }
}
</script>